...
class AnonymizeCaseData(ToolkitWorkflow):
    ...
    def analyze_synthesizability(self, df: pd.DataFrame) -> SynthesizabilityStatistics:
        ...
        for col in df.columns.to_numpy():
            distinct_values = [x for x in df[col].astype(str).unique() if x not in ["", "nan"]]
        ...
        mean_vals_per_record = sum([len([y for y in x if str(y) not in ['nan', '']]) for x in df.to_numpy()]) / df.shape[0]
    ...
    def anonymize_case_data(
        ...
        self.synthetic_df = Dataset.raw_data_to_data_frame(
            synthetic_raw_data
        )
